"""
Script 22: XGBoost with 10 repeated train-test splits on all data
"""
...
def main():
    print("="*60)
    print("Script 22: XGBoost Repeated Train-Test on All Data")
    print("="*60)
    ...
    hyperparams_df.to_csv('alldata_repeated_xgb_results.csv', index=False)
    
    mean_mae = np.mean(all_maes)
    std_mae = np.std(all_maes)
    
    print(f"\nXGBoost All Data Repeated Train-Test Results:")
    print(f"Mean MAE: {mean_mae*100:.2f}% ± {std_mae*100:.2f}%")
    print(f"Results saved to: alldata_repeated_xgb_results.csv")
