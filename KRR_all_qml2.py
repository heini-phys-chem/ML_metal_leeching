...
#                # First solve L * z = y for z
#                z = cp.linalg.solve(L, y_train)
#
#                # Then solve L.T * alpha = z for alpha
#                alpha = cp.linalg.solve(L.T, z)
...
def get_predictions(X_train, X_test, y_train, sigma, llambda):
    ...
    #print(f"kernel diffs: {np.mean(np.abs(K_train-K_train_gpu))}")
    ...
    #alpha = svd_solve(C, y_train)
    alpha = cp.linalg.solve(C, y_train)
    #alpha = cp.linalg.cholesky(C, y_train)
    # Perform Cholesky decomposition
#    L = cp.linalg.cholesky(C)
#
#    # Solve for alpha using forward and backward substitution
#    # First solve L * z = y for z
#    z = cp.linalg.solve(C, y_train)
#
#    # Then solve L.T * alpha = z for alpha
#    alpha = cp.linalg.solve(L.T, z)
...
#    best_sigma = 5.12
#    best_lambda = 1e-4
    #tqdm.write(f"{best_sigma}, {best_lambda}")
...
def get_scatterplot(actual_values, predicted_values, annotations, ax, maes, null, metal_labels):

    unique_metals = list(set(metal_labels))
    colors = plt.cm.viridis(np.linspace(0, 1, len(unique_metals)))
    color_map = dict(zip(unique_metals, colors))

    for metal in unique_metals:
        idx = np.array(metal_labels) == metal
        ax.scatter(actual_values[idx], predicted_values[idx], color=color_map[metal], label=metal, s=200)
...
#    ax.set_ylabel('Predicted Values')
    ...
    # Add a title
    ax.set_title(r'MAE: {:.2f}%, Null: {:.2f}%,  R$^2$: {:.2f}'.format(maes, null, r2_score(actual_values, predicted_values)))
...
def main():
    ...
    annotations = df['Sample name'].to_numpy()
    ...
    for i in tqdm(range(X.shape[0])):
        X_train, X_test = np.concatenate((X_scaled[:i], X_scaled[i+1:])), np.asarray([X_scaled[i]])
        y_train, y_test = np.concatenate((Y[:i], Y[i+1:])), Y[i]
        y_test = np.asarray(y_test)

        predictions = get_KRR(X_train, X_test, y_train, y_test)
        if predictions > 1.: predictions = 1.
        preds = np.append(preds, predictions)
        real = np.append(real, y_test)
        maes = np.append(maes, np.mean(np.abs(predictions-y_test)))
        null_maes = np.append(null_maes, np.mean(np.abs( np.mean(y_train)-y_test  )   )  )

    print("MAE: {}".format(np.mean(maes)*100))
    print("Null model: {}".format(np.mean(null_maes)*100))

    get_scatterplot(real, preds, annotations, ax, np.mean(maes)*100, np.mean(null_maes)*100, metal_labels)
    #print("Uncertainty: {}".format(np.mean(df['sd_' + prop].values)))
#    np.savez("predictions.npz", preds)
#    np.savez("Ys.npz", Y)
    plt.tight_layout()
    fig.savefig("tmp2.png", dpi=300)
    plt.show()
