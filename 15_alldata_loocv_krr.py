"""
Script 15: KRR with LOOCV on all data (all_data.xlsx)
"""
...
def main():
    print("="*60)
    print("Script 15: KRR LOOCV on All Data")
    print("="*60)
    ...
    # Save results
    np.savez('alldata_loocv_krr_results.npz',
             predictions=predictions_all,
             actuals=actuals_all,
             metal_labels=metal_labels)
    
    hyperparams_df = pd.DataFrame(hyperparams_list)
    hyperparams_df.to_csv('alldata_loocv_krr_hyperparameters.csv', index=False)
    
    mae = np.mean(np.abs(predictions_all - actuals_all))
    print(f"\nKRR All Data LOOCV MAE: {mae*100:.2f}%")
    print(f"Results saved to: alldata_loocv_krr_results.npz")
    print(f"Hyperparameters saved to: alldata_loocv_krr_hyperparameters.csv")
