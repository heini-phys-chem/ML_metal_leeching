"""
Script 23_oof: Ensemble (KRR+XGB) with OOF predictions - 10 repeated train-test splits
All data: all_data.xlsx
"""
...
def main():
    print("="*60)
    print("Script 23_oof: Ensemble (OOF) Repeated Splits - All Data")
    print("="*60)
    ...
    results_df.to_csv('alldata_repeated_ensemble_oof_results.csv', index=False)
    
    mean_mae = np.mean(all_maes)
    std_mae = np.std(all_maes)
    
    print(f"\nEnsemble (OOF) Repeated Splits MAE: {mean_mae*100:.2f}% ± {std_mae*100:.2f}%")
    print(f"Results saved to: alldata_repeated_ensemble_oof_results.csv")
