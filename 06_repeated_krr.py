"""
Script 06: KRR with 10 repeated train-test splits for transferability testing
"""
...
def main():
    print("="*60)
    print("Script 06: KRR Repeated Train-Test Splits (10 iterations)")
    print("="*60)
    ...
    hyperparams_df.to_csv('repeated_krr_results.csv', index=False)
    
    mean_mae = np.mean(all_maes)
    std_mae = np.std(all_maes)
    
    print(f"\nKRR Repeated Train-Test Results:")
    print(f"Mean MAE: {mean_mae*100:.2f}% ± {std_mae*100:.2f}%")
    print(f"Results saved to: repeated_krr_results.csv")
