"""
Script 02: Random Forest with LOOCV on training data (MS_samples_Al.xlsx)
"""
...
def main():
    print("="*60)
    print("Script 02: Random Forest LOOCV on Training Data")
    print("="*60)
    ...
    # Save results
    np.savez('loocv_rf_results.npz',
             predictions=predictions_all,
             actuals=actuals_all,
             metal_labels=metal_labels)
    
    hyperparams_df = pd.DataFrame(hyperparams_list)
    hyperparams_df.to_csv('loocv_rf_hyperparameters.csv', index=False)
    
    mae = np.mean(np.abs(predictions_all - actuals_all))
    print(f"\nRF LOOCV MAE: {mae*100:.2f}%")
    print(f"Results saved to: loocv_rf_results.npz")
    print(f"Hyperparameters saved to: loocv_rf_hyperparameters.csv")
